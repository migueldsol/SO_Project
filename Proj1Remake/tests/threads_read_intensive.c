...
void *threads_read(void *path_name){
    ...
    assert(memcmp(write_contets, read_contets, write_len) == 0);        //verifica se a leitura esta correta
...
int main(){
    ...
    pthread_t *threads = malloc(sizeof(pthread_t) * 5000);      //cria uma tabela de 5000 threads
    char path_name[] = "/f1";
    ...
    for (int i = 0; i < 5000; i++){
        assert(pthread_create(&(threads[i]), NULL, &threads_read, (void*)path_name) == 0);  //cria 5000 threads para escrever no ficheiro
    }
    
    for (int i = 0; i < 5000; i++){
            assert(pthread_join(threads[i], NULL) == 0);        //espera que estas retornem
    }
