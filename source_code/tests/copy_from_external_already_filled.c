...
int main() {
    ...
    int fd = tfs_open(new_file_path, TFS_O_CREAT);  // creates a new file nammed newFile
    assert(fd!=-1);

    assert(tfs_write(fd, str_new_file, new_file_len) != -1);  //writes big text in newFile

    f = tfs_copy_from_external_fs(path_src, new_file_path);     //copies small text from path_src to new_file_path
    assert(f != -1);
